...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    size_t total_num_samps, spp, rx_chan, tx_chan, threshold, pulsewidth, delay, delay_2, warmup;
    double rate, rx_freq, tx_freq, rx_gain, tx_gain, rx_bw, tx_bw, total_time, setup_time, pulse2_gain;
    bool rx_timestamps;
    ...
    desc.add_options()
        ...
        ("spp", po::value<size_t>(&spp)->default_value(200), "Samples per packet (reduce for lower latency)")
        ("threshold", po::value<size_t>(&threshold)->default_value(50), "Samples per packet (reduce for lower latency)")
        ("pw", po::value<size_t>(&pulsewidth)->default_value(100), "Samples per packet (reduce for lower latency)")
        ("delay", po::value<size_t>(&delay)->default_value(200), "Samples per packet (reduce for lower latency)")
        ...
        ("delay2", po::value<size_t>(&delay_2)->default_value(100), "Delay before second pulse in ce_clk cycles")
        ("pulse2-gain", po::value<double>(&pulse2_gain)->default_value(1.0), "Second pulse gain scale in [0.0, 1.0]")
        ("warmup", po::value<size_t>(&warmup)->default_value(0), "Warmup in ce_clk cycles")
...
siggen->set_threshold(threshold, port);
siggen->set_pulsewidth(pulsewidth, port);
siggen->set_delay(delay, port);
siggen->set_warmup(warmup, port);
siggen->set_delay_2(delay_2, port);
siggen->set_pulse2_gain(pulse2_gain, port);
...
std::cout << "delay       = " << siggen->get_delay(port) << " ce_clk cycles" << std::endl;
std::cout << "warmup      = " << siggen->get_warmup(port) << " ce_clk cycles" << std::endl;
std::cout << "delay_2     = " << siggen->get_delay_2(port) << " ce_clk cycles" << std::endl;
std::cout << "pulse2_gain = " << siggen->get_pulse2_gain(port) << std::endl;
