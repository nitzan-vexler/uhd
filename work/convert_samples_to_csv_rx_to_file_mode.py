...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", default=BINARY_FILE, help="Input binary .dat file")
    parser.add_argument("--output-folder", default=None, help="Output CSV folder")
    ...
    if args.output_folder is None:
        output_folder = os.path.join(os.path.dirname(input_file), "csv_output")
    else:
        output_folder = os.path.abspath(os.path.expanduser(args.output_folder))

    read_samples_binary_to_csv(
        input_file,
        output_folder,
        args.rows_per_file
    )
