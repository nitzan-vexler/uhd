...
assign s_tready = 1'b1;   // always ready to sample input for trigger detection
  ...
  reg [REG_DELAY_2_LEN-1:0]     reg_delay_2 = 32'd0;  // example default
  reg [REG_WARMUP_LEN-1:0]    reg_warmup;
  reg [REG_PULSE2_GAIN_LEN-1:0] reg_pulse2_gain = 16'h7FFF;
    ...
    if (rst) begin
      ...
      if (s_ctrlport_req_wr) begin
        ...
        case (s_ctrlport_req_addr)
          ...
          REG_DELAY_2    : reg_delay_2    <= s_ctrlport_req_data[REG_DELAY_2_LEN-1:0];
          REG_PULSE2_GAIN : reg_pulse2_gain <= s_ctrlport_req_data[REG_PULSE2_GAIN_LEN-1:0];
          REG_PHASE_INC : begin
      ...
      if (s_ctrlport_req_rd) begin
        ...
        case (s_ctrlport_req_addr)
          ...
          REG_DELAY_2    : s_ctrlport_resp_data[REG_DELAY_2_LEN-1:0] <= reg_delay_2;
          REG_PULSE2_GAIN : s_ctrlport_resp_data[REG_PULSE2_GAIN_LEN-1:0] <= reg_pulse2_gain;
...
wire [15:0] abs_i = in_i[15] ? (~in_i + 16'd1) : in_i;
wire [15:0] abs_q = in_q[15] ? (~in_q + 16'd1) : in_q;

// Cheap magnitude estimate
wire [15:0] trig_mag_est = (abs_i > abs_q) ? abs_i : abs_q;
...
reg [15:0] saved_trig_mag;
reg [15:0] pulse2_gain_latched;

// FSM-controlled burst flag
reg fsm_burst_active;
reg [15:0] trig_mag_est_d;
...
wire [31:0] pulse2_gain_from_trig_mult =
    trig_mag_est_d * reg_pulse2_gain;
    
// Q1.15 style scaling
wire [15:0] pulse2_gain_from_trig =
    (pulse2_gain_from_trig_mult[30:15] > 16'h7FFF) ? 16'h7FFF :
                                                      pulse2_gain_from_trig_mult[30:15];
                                                      
always @(posedge clk) begin
  if (rst) begin
    trig_mag_est_d         <= 16'd0;
  end else begin
    trig_mag_est_d         <= trig_mag_est;
  end
end
...
always @(posedge clk) begin
  if (rst) begin
    ...
    burst_count     <= 0;
    saved_delay_2   <= 0;
    saved_trig_mag      <= 16'd0;
    pulse2_gain_latched <= 16'd0;
    ...
    case (state)
      ...
      ST_IDLE: begin
        ...
        if (trig_pulse) begin
          ...
          saved_trig_mag      <= trig_mag_est_d;
          pulse2_gain_latched <= pulse2_gain_from_trig;
...
// assign m_tlast = your_spp_tlast & gated_tvalid;
  ...
  // Gain
  //---------------------------------------------------------------------------
wire is_second_pulse = use_trigger && (state == ST_BURST) && (burst_count == 1);

// Pulse 1 = fixed reg_gain
// Pulse 2 = dynamic gain based on received amplitude
wire [15:0] selected_gain = is_second_pulse ? pulse2_gain_latched : reg_gain;
